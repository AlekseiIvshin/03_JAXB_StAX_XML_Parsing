...
public class JAXBParser implements Parser{
	...
	public void parse(Schema schema,String xmlLocation,String resultPath){
		...
		try{
			...
			Categories category = (Categories) unmarsheller.unmarshal(new FileInputStream(f));
				...
				for(SubcategoryType st : ct.subcategories.getSubcategory()){
					...
					for(ProductType pt: st.products.getProduct()){
						if(pt.count > 0){
							...
							deletedCount++;
							System.out.printf("Deleted: %s/%s/%s %s%n",ct.name,st.name,pt.manufacturer,pt.model);
						}
					}
